use std::net::{Ipv4Addr, SocketAddr, SocketAddrV4, UdpSocket, ToSocketAddrs};
...
pub struct Message{
    ...
    pub op_Id: u32, // normal: 0, elect: 1, announce: 2, load balancing: 3 // different messages to the server 
    pub op_Arg: Vec<u8>,
}
...
fn send_reply(socket: &UdpSocket, message: Message, src:String)
{
    let bytes = bincode::serialize(&message).unwrap(); 
    socket.send_to(&bytes, src).expect("couldn't send message");
}

pub fn handel_normal(mut mes: Message, src: SocketAddr, my_ID: Arc<u8>, servers_state: Arc<Mutex<Vec<bool>>>, servers_load: Arc<Mutex<Vec<u8>>>)
{
    //.... Process request .....
    //thread::sleep(Duration::from_millis(1000));
    let socket = UdpSocket::bind("0.0.0.0:0").expect("couldn't bind to address");
    
    if servers_load.lock().unwrap()[*my_ID as usize] >= 2 && mes.op_Id != 5
    {
        for i in 0..NUMBER_SERVERS
        {
            if servers_state.lock().unwrap()[i]
            {
                if servers_load.lock().unwrap()[i] < 2
                {
                    mes.op_Id = 5;
                    let bytes = bincode::serialize(&mes).unwrap(); 
                    socket.send_to(&bytes, SERVER_LIST[i]).expect("couldn't send message");
                    return;
                }
            }
        }
...
pub fn handle_LB(mut mes: Message, src: SocketAddr, my_ID: Arc<u8>, servers_load: Arc<Mutex<Vec<u8>>>)
{
    let sender_id = mes.op_Arg[0] as usize;
    let new_load = mes.op_Arg[1];
        
    servers_load.lock().unwrap()[*my_ID as usize] = new_load;
}
